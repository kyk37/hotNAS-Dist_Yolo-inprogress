...
#FIXME from tensorflow.keras.utils import multi_gpu_model
...
#FIXME from tensorflow_model_optimization.sparsity import keras as sparsity
...
def main(args):
    ...
    log_dir = os.path.join(args.log_dir, datetime.now().strftime("%Y_%m_%d_%H_%M_%S"))
    ...
    if args.val_annotation_file:
        val_dataset = get_dataset(args.val_annotation_file, dataset_working_directory)
    ...
    if args.model_pruning:
        #FIXME pruning_callbacks = [sparsity.UpdatePruningStep(), sparsity.PruningSummaries(log_dir=log_dir, profile_batch=0)]
...
#        model = get_train_model(args.model_type, anchors, num_classes, weights_path=args.weights_path, freeze_level=freeze_level, optimizer=optimizer, label_smoothing=args.label_smoothing, elim_grid_sense=args.elim_grid_sense, model_pruning=args.model_pruning, pruning_end_step=pruning_end_step)
    ...
    # Finally store model
    if args.model_pruning:
        #FIXME model = sparsity.strip_pruning(model)
        raise "Not resolved, not supported now"
...
if __name__ == '__main__':
    ...
    # ENG ADDED:
    # Logging
    parser.add_argument("--log_dir", default="log", help="Location of logging info. Date/time subfolder will be added.")
    parser.add_argument("--checkpoint_period", default=1, type=int, help="Period of checkpoint saves.")
    parser.add_argument("--dataset_working_directory", default="", type=str, help="A relative directory to which dataset image names will be evaluated.")
