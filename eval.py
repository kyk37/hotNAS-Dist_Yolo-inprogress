...
def annotation_parse(annotation_lines, class_names):
    ...
    classes_records = {
        'car': [
                ['000001.jpg','100,120,200,235', '130'],
                ['000002.jpg','85,63,156,128', '12'],
                ...
               ],
        ...
    }
    ...
    for line in annotation_lines:
        ...
        for box in boxes:
            ...
            box_split = box.split(',')
            class_name = class_names[int(box_split[4])]
            coordinate = ','.join(box_split[0:4])
            distance = int(box_split[5])
            box_records[coordinate] = class_name
            # append or add ground truth class item
            record = [os.path.basename(image_name), coordinate, distance]
...
def yolo_predict_mnn(interpreter, session, image, anchors, num_classes, conf_threshold, elim_grid_sense):
    ...
    def get_tensor_list(output_tensors):
        ...
        for (output_tensor_name, output_tensor) in output_tensors.items():
            ...
            tensor_height, tensor_width = tensor_shape[
                                          2:4] if dim_type == MNN.Tensor_DimensionType_Caffe else tensor_shape[1:3]
    ...
    for (output_tensor_name, output_tensor) in output_tensor_list:
        ...
        tmp_output = MNN.Tensor(output_shape, output_tensor.getDataType(), \
                                ...
                                tuple(np.zeros(output_shape, dtype=float).reshape(output_elementsize, -1)),
...
def get_prediction_class_records(model, model_format, annotation_records, anchors, class_names, model_image_size,
                                 conf_threshold, elim_grid_sense, save_result):
    '''
    ...
    for (image_name, gt_records) in annotation_records.items():
        ...
        if model_format == 'TFLITE':
            pred_boxes, pred_classes, pred_scores = yolo_predict_tflite(model, image, anchors, len(class_names),
                                                                        conf_threshold, elim_grid_sense)
        # support of MNN model
        elif model_format == 'MNN':
            pred_boxes, pred_classes, pred_scores = yolo_predict_mnn(model, session, image, anchors, len(class_names),
                                                                     conf_threshold, elim_grid_sense)
        # support of TF 1.x frozen pb model
        elif model_format == 'PB':
            pred_boxes, pred_classes, pred_scores = yolo_predict_pb(model, image, anchors, len(class_names),
                                                                    model_image_size, conf_threshold, elim_grid_sense)
        # support of ONNX model
        elif model_format == 'ONNX':
            pred_boxes, pred_classes, pred_scores = yolo_predict_onnx(model, image, anchors, len(class_names),
                                                                      conf_threshold, elim_grid_sense)
        # normal keras h5 model
        elif model_format == 'H5':
            pred_boxes, pred_classes, pred_scores, pred_distances = yolo_predict_keras(model, image, anchors,
                                                                                       len(class_names),
        ...
        for box, cls, score, distance in zip(pred_boxes, pred_classes, pred_scores, pred_distances):
            pred_class_name = class_names[cls]
            xmin, ymin, xmax, ymax = box
            coordinate = "{},{},{},{}".format(xmin, ymin, xmax, ymax)

            # append or add predict class item
            record = [os.path.basename(image_name), coordinate, score, distance[0]]
...
def box_iou(pred_box, gt_box):
    ...
    inter_box = [max(pred_box[0], gt_box[0]), max(pred_box[1], gt_box[1]), min(pred_box[2], gt_box[2]),
...
def match_gt_box(pred_record, gt_records, iou_threshold=0.5):
    ...
    pred_box = [float(x) for x in pred_record[COORDINATES_INDEX].split(',')]

    for i, gt_record in enumerate(gt_records):
        # get ground truth box coordinate
        gt_box = [float(x) for x in gt_record[COORDINATES_INDEX].split(',')]
        iou = box_iou(pred_box, gt_box)

        # if the ground truth has been assigned to other
        # prediction, we couldn't reuse it
        if iou > max_iou and gt_record[USED_FLAG_INDEX] == 'unused' and pred_record[FILENAME_INDEX] == gt_record[
            FILENAME_INDEX]:
            max_iou = iou
            max_index = i
...
def calc_AP(gt_records, pred_records, class_name, iou_threshold, show_result):
    ...
    for idx, pred_record in enumerate(pred_records):
        ...
        result_record = {
            "file": pred_record[0],
            "pred_box": [int(q) for q in pred_record[1].split(',')],
            "conf": pred_record[2],
            "pred_dist": pred_record[3],
            "class": class_name,
            "iou_threshold": iou_threshold
        }
        result_set.append(result_record)

        # filter out gt record from same image
        image_gt_records = [gt_record for gt_record in gt_records if
                            gt_record[FILENAME_INDEX] == pred_record[FILENAME_INDEX]]
        ...
        if i != -1:
            ...
            # reference list
            image_gt_records[i][USED_FLAG_INDEX] = 'used'
            true_positive[idx] = 1
            true_positive_count += 1
            result_record["gt_box"] = [int(q) for q in image_gt_records[i][1].split(',')]
            result_record["gt_dist"] = image_gt_records[i][DIST_INDEX]
        else:
    ...
    # this prints result for every box into the file
    # with open(f"D:/dist_recs.txt", "a") as f:
    #     for dr in distance_results:
    #         f.write(f"{class_name};{iou_threshold};{dr[0]};{dr[1]}\n")

    return ap, true_positive_count, result_set
...
def plot_Pascal_AP_result(count_images, count_true_positives, num_classes,
    ...
    draw_plot_func(gt_counter_per_class, num_classes, window_title, plot_title, x_label, output_path, to_show=False,
                   plot_color='forestgreen', true_p_bar='')
    ...
    draw_plot_func(pred_counter_per_class, len(pred_counter_per_class), window_title, plot_title, x_label, output_path,
                   to_show=False, plot_color='forestgreen', true_p_bar=count_true_positives)
    ...
    draw_plot_func(APs, num_classes, window_title, plot_title, x_label, output_path, to_show=False,
    ...
    draw_plot_func(precision_dict, len(precision_dict), window_title, plot_title, x_label, output_path, to_show=False,
                   plot_color='royalblue', true_p_bar='')
    ...
    draw_plot_func(recall_dict, len(recall_dict), window_title, plot_title, x_label, output_path, to_show=False,
                   plot_color='royalblue', true_p_bar='')
...
def compute_mAP_PascalVOC(annotation_records, gt_classes_records, pred_classes_records, class_names, iou_threshold,
                          show_result=True):
    '''
    ...
    for _, class_name in enumerate(class_names):
        ...
        ap, true_positive_count, class_result_set = calc_AP(gt_records, pred_records, class_name, iou_threshold,
                                                            show_result)
        result_set.extend(class_result_set)

        APs[class_name] = ap
        count_true_positives[class_name] = true_positive_count
        distance_deltas = [r["pred_dist"] - r["gt_dist"] for r in class_result_set if "gt_dist" in r]
        dists[class_name] = [true_positive_count, np.min(distance_deltas), np.mean(distance_deltas),
                             np.mean(np.abs(distance_deltas)),
                             np.max(distance_deltas)] \
            if len(distance_deltas) > 0 else [0, math.nan, math.nan, math.nan, math.nan]
    ...
    if show_result:
        ...
        for (class_name, dist) in dists.items():
            print('%s: dist cnt/min/mean/abs-mean/max = %.0f/%.4f/%.4f/%.4f/%.4f' % (
                class_name, dist[0], dist[1], dist[2], dist[3], dist[4]))

    # return mAP percentage value
    return mAP, APs, result_set
...
def compute_AP_COCO(annotation_records, gt_classes_records, pred_classes_records, class_names, show_result=True):
    ...
    for iou_threshold in iou_threshold_list:
        ...
        mAP, _, threshold_result_set = compute_mAP_PascalVOC(annotation_records, gt_classes_records,
                                                             pred_classes_records,
                                                             class_names, iou_threshold, show_result=False)
        APs[iou_threshold] = round(mAP, 6)
        result_set.extend(threshold_result_set)
        pbar.update(1)
    ...
    if show_result:
        ...
        draw_plot_func(APs, len(APs), window_title, plot_title, x_label, output_path, to_show=False,
                       plot_color='royalblue', true_p_bar='')
        ...
        # print distance to ouput
        print("\nDist evaluation")
        for iou_threshold in iou_threshold_list:
            tmp = [f for f in result_set if f["iou_threshold"] == iou_threshold and "gt_dist" in f]
            _print_result_set_summary(tmp, f"Summary for threshold {iou_threshold:.1f} :::")
            for class_name in class_names:
                clstmp = [f for f in tmp if f["class"] == class_name]
                _print_result_set_summary(clstmp,
                                          f"Summary for threshold {iou_threshold:.1f} and class {class_name} :::")

    # return AP percentage value
    return AP, APs, result_set


def _print_result_set_summary(result_set, label=None):
    tmp = [r["pred_dist"] - r["gt_dist"] for r in result_set]
    cnt = len(tmp)
    if cnt == 0:
        print(label, "No box found")
        return

    min = np.min(tmp)
    max = np.max(tmp)
    mean = np.mean(tmp)
    abs_mean = np.mean(np.abs(tmp))

    s = f"dist cnt/min/mean/abs-mean/max: {cnt}/{min:.3f}/{mean:.3f}/{abs_mean:.3f}/{max:.3f}"
    if label:
        print(label, s)
    else:
        print(s)
...
def compute_AP_COCO_Scale(annotation_records, scale_gt_classes_records, pred_classes_records, class_names):
    ...
    for scale_key in ['small', 'medium', 'large']:
        ...
        scale_AP, _, _ = compute_AP_COCO(annotation_records, gt_classes_records, pred_classes_records, class_names,
                                         show_result=False)
    ...
    draw_plot_func(scale_APs, len(scale_APs), window_title, plot_title, x_label, output_path, to_show=False,
                   plot_color='royalblue', true_p_bar='')
    ...
    for scale_key in ['small', 'medium', 'large']:
        ...
        for _, class_name in enumerate(class_names):
            ...
            gt_classes_sum[class_name] = np.sum(
                len(gt_classes_records[class_name])) if class_name in gt_classes_records else 0
        ...
        draw_plot_func(gt_classes_sum, len(gt_classes_sum), window_title, plot_title, x_label, output_path,
                       to_show=False, plot_color='royalblue', true_p_bar='')
...
def get_scale_gt_dict(gt_classes_records, class_names):
    ...
    for _, class_name in enumerate(class_names):
        ...
        for (image_file, box, dist) in gt_records:
            ...
            if box_area <= 32 * 32:
                small_gt_records = add_gt_record(small_gt_records, [image_file, box, dist], class_name)
            elif box_area > 32 * 32 and box_area <= 96 * 96:
                medium_gt_records = add_gt_record(medium_gt_records, [image_file, box, dist], class_name)
            elif box_area > 96 * 96:
                large_gt_records = add_gt_record(large_gt_records, [image_file, box, dist], class_name)
...
def eval_AP(model, model_format, annotation_lines, anchors, class_names, model_image_size, eval_type, iou_threshold,
    ...
    pred_classes_records = get_prediction_class_records(model, model_format, annotation_records, anchors, class_names,
                                                        model_image_size, conf_threshold, elim_grid_sense, save_result)
    AP = 0.0

    if eval_type == 'VOC':
        AP, _, result_set = compute_mAP_PascalVOC(annotation_records, gt_classes_records, pred_classes_records,
                                                  class_names,
                                                  iou_threshold)
    elif eval_type == 'COCO':
        AP, _, result_set = compute_AP_COCO(annotation_records, gt_classes_records, pred_classes_records, class_names)
    ...
    return AP, result_set

def _save_result_json(result_set, output_file_name):
    with open(output_file_name, "w") as f:
        json.dump(result_set, f)

def _save_result_set(result_set, output_file_name):
    with open(output_file_name, "w") as result_file:
        for r in result_set:
            if "gt_dist" not in r: continue
            error_ratio = r["pred_dist"] - r["gt_dist"]
            gt_error_ratio = abs(error_ratio) if r["gt_dist"] <= 0 else abs(error_ratio / r["gt_dist"])
            s = f"{r['file']};{r['class']};{r['iou_threshold']};{r['gt_dist']};{r['pred_dist']:.3f};{error_ratio:.3f};{gt_error_ratio:.3f}"
            for p in r['gt_box']:
                s += f";{p:.0f}"
            for p in r['pred_box']:
                s += f";{p:.0f}"
...
def _draw_result_set(result_set, dataset_working_directory, output_directory, image_scale=1.0):
    files = []
    classes = []
    for r in result_set:
    ...
    for file in files:
        image = Image.open(os.path.join(dataset_working_directory, file))

        if image.mode != 'RGB':
            image = image.convert('RGB')

        if image_scale != 1:
            new_size = np.array(image.size) * image_scale
            new_size = (int(new_size[0]), int(new_size[1]))
            image = image.resize(new_size)
        image_array = np.array(image, dtype='uint8')

        tmp = [r for r in result_set if r["file"] == file]

        # to draw gt boxes, only add them into _draw_boxes3 as a parameter
        # gt_boxes = [r["gt_box"] for r in tmp]
        pred_boxes = [r["pred_box"] for r in tmp]
        if image_scale != 1:
            pred_boxes = (np.array(pred_boxes) * image_scale).astype('int')
        classes = [r["class"] for r in tmp]
        colors = [class_color_map[c["class"]] for c in tmp]
        labels = [f"{c['class']} {c['pred_dist']:.0f}/{c['gt_dist']}" for c in tmp]

        image_array = _draw_boxes3(image_array, pred_boxes, classes, colors, drawn_labels=labels)

        image = Image.fromarray(image_array)
        output_image_filename = os.path.join(output_directory, file)
        # os.makedirs(Path(output_image_filename).parent, exist_ok=True)
...
def main():
    ...
    parser = argparse.ArgumentParser(argument_default=argparse.SUPPRESS,
                                     description='evaluate YOLO model (h5/pb/onnx/tflite/mnn) with test dataset')
    '''
    ...
    # ENG
    parser.add_argument("--dataset_working_directory", default="", type=str,
                        help="A relative directory to which dataset image names will be evaluated.")
    ...
    model_image_size = (int(height), int(width))
    assert (model_image_size[0] % 32 == 0 and model_image_size[
        1] % 32 == 0), 'model_image_size should be multiples of 32'
    ...
    annotation_lines = get_dataset(annotation_file, dataset_working_directory, shuffle=False)
    model, model_format = load_eval_model(args.model_path)

    start = time.time()
    ap, result_set = eval_AP(model, model_format, annotation_lines, anchors, class_names, model_image_size,
                             args.eval_type,
                             args.iou_threshold, args.conf_threshold, args.elim_grid_sense, args.save_result)
    end = time.time()
    print("Evaluation time cost: {:.6f}s".format(end - start))
    if args.save_result:
        tmp = [r for r in result_set if r["iou_threshold"] == 0.5 and "gt_box" in r]
        _draw_result_set(tmp, dataset_working_directory, "D:/results", image_scale=2.0)
        _save_result_set(result_set, "D:/results/_result.txt")
        _save_result_json(result_set, "D:/results/_result.json")
