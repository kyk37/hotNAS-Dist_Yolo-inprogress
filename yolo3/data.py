...
def get_ground_truth_data(annotation_line, input_shape, augment_mode, max_boxes=100):
    ...
    # suppressed
    if augment_mode == AugmentMode.NO_PADDING_SCALING:
        image, padding_size, padding_offset = letterbox_resize(image, target_size=model_input_size,
                                                               return_padding_info=True)
    else:
    ...
    # supressed # image = random_grayscale(image)
...
def yolo3_data_generator(annotation_lines, batch_size, input_shape, anchors, num_classes, enhance_augment,
    ...
    while True:
        ...
        for b in range(batch_size):
            ...
            image, box = get_ground_truth_data(annotation_lines[i], input_shape, augment_mode=AugmentMode.ALL)
            image_data.append(image)
            box_data.append(box)
            i = (i + 1) % n
        image_data = np.array(image_data)
