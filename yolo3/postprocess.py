...
def batched_yolo3_prenms(args,
    ...
    for l in range(num_layers):
        ...
        box_wh = torch.exp(reshape_feats[..., 2:4]) * anchors_tensor / torch.tensor(input_shape[::-1], dtype=reshape_feats)
        ...
        input_shape = torch.tensor(input_shape, dtype=box_xy)
        ...
        new_shape = torch.tensor(new_shape, dtype='float32')
        offset = (input_shape-new_shape)/2./input_shape
        scale = input_shape/new_shape
        box_xy = (box_xy - offset) * scale
        box_wh *= scale
        ...
        _box_scores = torch.reshape(_box_scores, [-1, total_anchor_num, num_classes])
