...
def yolo3_decode(feats, anchors, num_classes, input_shape, scale_x_y=None, calc_loss=False):
    ...
    grid_shape = torch.shape(feats)[1:3] # height, width
    grid_y = torch.tile(torch.reshape(torch.arange(0, stop=grid_shape[0]), [-1, 1, 1, 1]),
        [1, grid_shape[1], 1, 1])
    grid_x = torch.tile(torch.reshape(torch.arange(0, stop=grid_shape[1]), [1, -1, 1, 1]),
        [grid_shape[0], 1, 1, 1])
    grid = torch.concatenate([grid_x, grid_y])
    grid = torch.tensor(grid, dtype=feats) #grid.type(feats.dtype)

    feats = torch.reshape(
        feats, [-1, grid_shape[0], grid_shape[1], num_anchors, num_classes + 5 + 1])
    ...
    if scale_x_y:
        ...
        #
        box_xy_tmp = torch.sigmoid(feats[..., :2]) * scale_x_y - (scale_x_y - 1) / 2
        box_xy = (box_xy_tmp + grid) / torch.tensor(grid_shape[..., ::-1],dtype=feats) #K.cast(grid_shape[..., ::-1], K.dtype(feats))
    else:
        box_xy = (torch.sigmoid(feats[..., :2]) + grid) / torch.tensor(grid_shape[...,::-1],dtype=feats) #K.cast(grid_shape[..., ::-1], K.dtype(feats))
    box_wh = torch.exp(feats[..., 2:4]) * anchors_tensor / torch.tensor(input_shape[...,::-1], dtype=feats) #K.cast(input_shape[..., ::-1], K.dtype(feats))
    box_confidence = torch.sigmoid(feats[..., 4:5])
    box_class_probs = torch.sigmoid(feats[..., 5:5+num_classes])
...
def yolo3_correct_boxes(box_xy, box_wh, input_shape, image_shape):
    '''Get corrected boxes'''
    input_shape = torch.tensor(input_shape, dtype = box_xy) # K.cast(input_shape, K.dtype(box_xy))
    image_shape = torch.tensor(input_shape, dtype = box_xy)# K.cast(image_shape, K.dtype(box_xy))

    #reshape the image_shape tensor to align with boxes dimension
    image_shape = torch.reshape(image_shape, [-1, 1, 1, 1, 2])
    ...
    boxes *= torch.concatenate([image_wh, image_wh])
    return boxes
...
def yolo3_boxes_and_scores(feats, anchors, num_classes, input_shape, image_shape, scale_x_y):
    ...
    box_scores = torch.reshape(box_scores, [-1, num_classes])
...
def yolo3_postprocess(args,
    ...
    boxes = torch.concatenate(boxes, axis=0)
    box_scores = torch.concatenate(box_scores, axis=0)

    mask = box_scores >= confidence
    max_boxes_tensor = torch.tensor(max_boxes, dtype='int32')
    boxes_ = []
    scores_ = []
    classes_ = []
    for c in range(num_classes):
        # TODO: use keras backend instead of tf.
        class_boxes = torch.masked_select(boxes, mask[:, c])
        class_box_scores = torch.masked_select(box_scores[:, c], mask[:, c])
        nms_index = ops.nms(
            class_boxes, class_box_scores, max_boxes_tensor, iou_threshold=iou_threshold)
        class_boxes = torch.gather(class_boxes, nms_index)
        class_box_scores = torch.gather(class_box_scores, nms_index)
        classes = torch.ones_like(class_box_scores, 'int32') * c
        boxes_.append(class_boxes)
        scores_.append(class_box_scores)
        classes_.append(classes)
    boxes_ = torch.concatenate(boxes_, axis=0)
    scores_ =  torch.concatenate(scores_, axis=0)
    classes_ = torch.concatenate(classes_, axis=0)
...
def batched_yolo3_boxes_and_scores(feats, anchors, num_classes, input_shape, image_shape, scale_x_y):
    ...
    num_anchors = len(anchors)
    grid_shape = feats.shape[1:3] # feats should be tensor
#     grid_shape = K.shape(feats)[1:3] # height, width
    total_anchor_num = grid_shape[0] * grid_shape[1] * num_anchors

    boxes = yolo3_correct_boxes(box_xy, box_wh, input_shape, image_shape)
    boxes = torch.reshape(boxes, [-1, total_anchor_num, 4])
    box_scores = box_confidence * box_class_probs
    box_scores = torch.reshape(box_scores, [-1, total_anchor_num, num_classes])
    return boxes, box_scores
...
def batched_yolo3_postprocess(args,
    ...
    batch_size = image_shape.shape[0] # batch size, tensor
    # print("yolo_outputs",yolo_outputs)
    boxes = []
    box_scores = []
    for l in range(num_layers):
        ...
        boxes.append(_boxes)
        box_scores.append(_box_scores)
    boxes = torch.concatenate(boxes, axis=1)
    box_scores = torch.concatenate(box_scores, axis=1)
...
#     max_boxes_tensor = K.constant(max_boxes, dtype='int32')
    ...
    def single_image_nms(b, batch_boxes, batch_scores, batch_classes):
        ...
        for c in range(num_classes):
            # TODO: use keras backend instead of tf.
            class_boxes = torch.masked_select(boxes[b], mask[b, :, c])
            class_box_scores = torch.mask_select(box_scores[b, :, c], mask[b, :, c])
            nms_index = ops.nms(
                class_boxes, class_box_scores, max_boxes_tensor, iou_threshold=iou_threshold)
            class_boxes = torch.gather(class_boxes, nms_index)
            class_box_scores = torch.gather(class_box_scores, nms_index)
    ...
    while b < batch_size:
        b, batch_boxes, batch_scores, batch_classes = single_image_nms(lambda b,*args: b<batch_size, single_image_nms, [0, batch_boxes, batch_scores, batch_classes])
...
def batched_yolo3_prenms(args,
    ...
    for l in range(num_layers):
        ...
        anchors_tensor = anchorset.view(1, 1, 1, num_anchors, 2).to(feats.device)

        total_anchor_num = grid_shape[0] * grid_shape[1] * num_anchors
        grid_y = torch.arange(0, grid_shape[0], dtype=torch.float32).view(-1, 1, 1, 1).repeat(1, grid_shape[1], 1, 1)
        grid_x = torch.arange(0, grid_shape[1], dtype=torch.float32).view(1, -1, 1, 1).repeat(grid_shape[0], 1, 1, 1)
        grid = torch.cat([grid_x, grid_y], dim=-1).to(feats.device)

        feats = feats.view(-1, grid_shape[0], grid_shape[1], num_anchors, num_classes + 5)

        box_xy = (torch.sigmoid(feats[..., :2]) + grid) / torch.tensor(grid_shape[::-1], dtype=torch.float32).to(feats.device)
        box_wh = torch.exp(feats[..., 2:4]) * anchors_tensor / torch.tensor(input_shape[::-1], dtype=torch.float32).to(feats.device)
        box_confidence = torch.sigmoid(feats[..., 4:5])
        box_class_probs = torch.sigmoid(feats[..., 5:])

        input_shape_tensor = torch.tensor(input_shape, dtype=torch.float32).to(feats.device)
        image_shape_tensor = torch.tensor(image_shape, dtype=torch.float32).to(feats.device)
        new_shape = torch.round(image_shape_tensor * torch.min(input_shape_tensor / image_shape_tensor, dim=0).values)
        offset = (input_shape_tensor - new_shape) / 2.0 / input_shape_tensor
        scale = input_shape_tensor / new_shape
        box_xy = (box_xy - offset) * scale
        box_wh *= scale
        ...
        _boxes *= torch.cat([image_shape_tensor, image_shape_tensor], dim=-1)

        _boxes = _boxes.view(-1, total_anchor_num, 4)
        _box_scores = (box_confidence * box_class_probs).view(-1, total_anchor_num, num_classes)
...
class Yolo3PostProcessLayer(nn.Module): #Layer):
    ...
    def call(self, x):
        ...
        batch_size = image_shape[0].shape # batch size, tensor
        ...
        for l in range(self.num_layers):
            ...
            num_anchors = len(anchorset)
            # Reshape to batch, height, width, num_anchors, box_params.
            anchors_tensor = torch.reshape(torch.tensor(anchorset), [1, 1, 1, num_anchors, 2])
            ...
            grid_x = torch.tile(torch.reshape(torch.arange(0, stop=grid_shape[1]), [1, -1, 1, 1]),
                [grid_shape[0], 1, 1, 1])
            grid = torch.concatenate([grid_x, grid_y])
            grid = torch.tensor(grid, dtype=feats)

            reshape_feats = torch.reshape(
                feats, [-1, grid_shape[0], grid_shape[1], num_anchors, self.num_classes + 5])

            # Adjust preditions to each spatial grid point and anchor size.
            box_xy = (torch.sigmoid(reshape_feats[..., :2]) + grid) / torch.tensor(grid_shape[::-1], dtype=reshape_feats)
            box_wh = torch.exp(reshape_feats[..., 2:4]) * anchors_tensor / torch.tensor(self.input_dim[::-1], dtype=reshape_feats)
            box_confidence = torch.sigmoid(reshape_feats[..., 4:5])
            box_class_probs = torch.sigmoid(reshape_feats[..., 5:])

            # correct boxes to the original image shape
            input_shape = torch.tensor(self.input_dim, dtype=box_xy)
            image_shape = torch.tensor(image_shape, dtype=box_xy)
            #new_shape = K.round(image_shape * K.min(input_shape/image_shape))
            new_shape = torch.tensor(image_shape * torch.min(input_shape/image_shape), dtype='int32')
            new_shape = torch(new_shape, dtype='float32')
            offset = (input_shape-new_shape)/2./input_shape
            scale = input_shape/new_shape
            box_xy = (box_xy - offset) * scale
            box_wh *= scale
            ...
            # Scale boxes back to original image shape.
            _boxes *= torch.concatenate([image_shape, image_shape])

            # Reshape boxes to flatten the boxes
            _boxes = torch.reshape(_boxes, [-1, total_anchor_num, 4])
            _box_scores = box_confidence * box_class_probs
            _box_scores = torch.reshape(_box_scores, [-1, total_anchor_num, self.num_classes])
